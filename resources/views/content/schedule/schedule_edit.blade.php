...
@section('main_content')
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-md-4">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            <h1 class="h2">Редактировать Расписания</h1>
            <a class="btn btn-secondary" href="{{ url('/schedule') }}">Назад</a>
        </div>
        <div>
            <form action="{{ url("/schedule/{$schedule->id}") }}" enctype="multipart/form-data" method="POST" onsubmit="return validateForm()" class="create-post-form">
                {{ method_field('PUT') }}
                @csrf
                <div class="form-row">
                ...
                <div class="form-row">
                    <div class="form-group col-md-12">
                        <textarea name="summernote" id="summernote">{{ $schedule->content ?? '' }}</textarea>
                ...
                <div class="form-row">
                    <div class="form-group col-md-12">
                        ...
                        <br>
                        <textarea class="form-control" name="notes" rows="5" cols="100" id="notes">{{ $schedule->notes ?? '' }}</textarea>
                ...
                <div class="form-row">
                    <div class="form-group col-md-12">
                        ...
                        <br>
                        <textarea class="col-12 col-lg-6" name="google_maps" id="google_maps" cols="150" rows="2">{{ $schedule->google_maps ?? '' }}</textarea>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group col-md-6">
                        <label for="price">Цена:</label>
                        <input type="number" class="form-control" name="price" id="price" placeholder="" value="{{ $schedule->price ?? '' }}">
                ...
                <div class="form-row">
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control" name="from_date" type="text" id="from_date" value="{{ $schedule->from_date ?? '' }}">
                ...
                <div class="form-row">
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control" name="to_date" type="text" id="to_date" value="{{ $schedule->to_date ?? '' }}">
                        <span class="text-danger to_date"></span>
                    </div>
                </div>
                @if($schedule->gallery)
                <div class="form-row">
                    @foreach ($schedule->gallery as $image)
                        <div class="card p-1 col-12 col-lg-2" style="width: 18rem;">
                            <img class="card-img-top" src="{{ url('images/schedule/' . $image) }}" alt="Card image cap">
                            <div class="card-body text-center">
                                <a href="#" data-id="{{ $schedule->id }}" data-image="{{ $image }}" class="non-target btn btn-danger delete-image">Удалить</a>
                            </div>
                        </div>
                    @endforeach
                </div>
...
<script>
    ...
    $('.delete-image').on('click', function(){
        ...
        sendAjax(data, UPLOAD_SCHEDULE_URL, false, 'get', 'html', deleteImage);
    })
